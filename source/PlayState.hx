...
class PlayState extends MusicBeatState
{
	...
	//	practiceMode = false;
		...
		case 'philly': //Week 3
			if(!ClientPrefs.lowQuality) {
				...
				for (i in 0...5)
				{
					var light:BGSprite = new BGSprite('backgrounds/philly/win' + i, city.x, city.y, 0.3, 0.3);
					light.visible = false;
				...
				CoolUtil.precacheSound('stage-specific/train_passes');
				FlxG.sound.list.add(trainSound);

				var street:BGSprite = new BGSprite('backgrounds/philly/street', -40, 50);
				add(street);
		...
		if (!ClientPrefs.cursing) Paths.txt(songName + '/' + songName + 'Dialogue-clean');
		if (OpenFlAssets.exists(file)) {
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				case "winter-horrorland":
					...
					FlxG.sound.play(Paths.sound('cutscenes/Lights_Turn_On'));
	...
	public function startCountdown():Void
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
			{
				...
				introAssets.set('pixel', ['ui/pixelUI/ready-pixel', 'ui/pixelUI/set-pixel', 'ui/pixelUI/date-pixel']);
				...
				{
					case 0:
						FlxG.sound.play(Paths.sound('countdowns/' + introSoundsSuffix + 'intro3'), 0.6);
						bopIcons();
					case 1:
						...
						FlxG.sound.play(Paths.sound('countdowns/' + introSoundsSuffix + 'intro2'), 0.6);
						bopIcons();
					case 2:
						...
						FlxG.sound.play(Paths.sound('countdowns/' + introSoundsSuffix + 'intro1'), 0.6);
						bopIcons();
					case 3:
						...
						FlxG.sound.play(Paths.sound('countdowns/' + introSoundsSuffix + 'introGo'), 0.6);
						bopIcons();
					case 4:
						bopIcons();
				}
	...
	override public function update(elapsed:Float)
	{
		...
		if(ratingString == '?') {
			scoreTxt.text = 'Score: 0 / Misses: 0 / Rating: N/A' + (practiceMode ? " / PRACTICE" : "");
		} else {
			scoreTxt.text = 'Score: ' + songScore + ' / Misses: ' + songMisses + ' / Rating: ' + ratingString + ' (' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%)' + (practiceMode ? " / PRACTICE" : "");
		}
		} else scoreTxt.text = 'Score: ' + songScore + (practiceMode ? " / PRACTICE" : "");
		...
		else if (healthBar.percent > 80 && boyfriend.hasWinningIcon)
		...
		else if (healthBar.percent < 20 && dad.hasWinningIcon)
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.mustPress && daNote.wasGoodHit && !daNote.hitByOpponent && !daNote.ignoreNote)
				{
					...
					if (ClientPrefs.cameraMovements) {
			...
			if (isStoryMode)
			{
				...
				else
				{
					...
					if (winterHorrorlandNext)
					{
						...
						FlxG.sound.play(Paths.sound('cutscenes/Lights_Shut_off'));
					}
	...
	private function keyShit():Void
	{
		...
		if (!boyfriend.stunned && generatedMusic)
		{
			...
			if ((controlHoldArray.contains(true) || controlArray.contains(true)) && !endingSong) {
				...
				if (ClientPrefs.ghostTapping == 0) canMiss = true;
					...
					for (i in 0...controlArray.length) {
						...
						notes.forEachAlive(function(daNote:Note)
						{
							...
							&& !daNote.wasGoodHit && daNote.noteData == i) {
								...
								if (ClientPrefs.ghostTapping == 1)
								canMiss = true;
								else if (ClientPrefs.ghostTapping == 2)
								canMiss = false;
							}
						});
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		if (daNote.isSustainNote) missedNote(true);
		else missedNote(false);
		...
		if (!note.wasGoodHit)
		{
			...
			if (!note.isSustainNote)
			{
				...
				if (ClientPrefs.hitsounds)
					FlxG.sound.play(Paths.sound('ChartingTick'), 0.7);
			}
	...
	function spawnNoteSplashOnNote2(daNote:Note) {
		if(ClientPrefs.noteSplashes && ClientPrefs.opponentSplashes && daNote != null) {
	...
	private function bopIcons():Void
		{
			...
			FlxTween.tween(iconP1, {"scale.x": 1, "scale.y": 1}, 0.4, {ease: FlxEase.cubeOut});
			iconP2.scale.set(1.3, 1.3);
			FlxTween.tween(iconP2, {"scale.x": 1, "scale.y": 1}, 0.4, {ease: FlxEase.cubeOut});
		}
