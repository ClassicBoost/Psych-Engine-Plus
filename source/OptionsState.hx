	...
	override function update(elapsed:Float)
	{
		...
		switch(options[curSelected]) {
			...
			case 'Display Accuracy':
				daText = 'If checked, your accuracy and misses will display';
			case 'Show Judgements':
			...
			case 'Ghost Tapping':
				...
				else if (ClientPrefs.ghostTapping == 1)
				daText = "You'll only miss if you press a invalid note while singing.";
				else
			...
			case 'Camera Movements':
				daText = "If checked, the notes will move the camera a bit.";
		...
		if(usesCheckbox) {
			if(controls.ACCEPT && nextAccept <= 0) {
				switch(options[curSelected]) {
					...
					case 'Camera Movements':
						ClientPrefs.cameraMovements = !ClientPrefs.cameraMovements;
					...
					case 'Show Judgements':
						ClientPrefs.showJudgements = !ClientPrefs.showJudgements;
		...
		for (i in 0...checkboxArray.length) {
			...
			if(checkbox != null) {
				...
				switch(options[checkboxNumber[i]]) {
					...
					case 'Show Judgements':
						daValue = ClientPrefs.showJudgements;
					case 'Display Accuracy':
				...
				//		daValue = ClientPrefs.ghostTapping;
					...
					case 'Camera Movements':
						daValue = ClientPrefs.cameraMovements;
					case 'Persistent Cached Data':
