...
class WeekEditorState extends MusicBeatState
{
	...
	function reloadBG() {
		...
		if(assetName != null && assetName.length > 0) {
			...
			Assets.exists(Paths.image('menus/storymenu/menubackgrounds/menu_' + assetName), IMAGE)) {
	...
	function reloadWeekThing() {
		...
		var isMissing:Bool = true;
		if(assetName != null && assetName.length > 0) {
			if( #if MODS_ALLOWED FileSystem.exists(Paths.modsImages('menus/storymenu/weeks/' + assetName)) || #end
			Assets.exists(Paths.image('menus/storymenu/weeks/' + assetName), IMAGE)) {
				weekThing.loadGraphic(Paths.image('menus/storymenu/weeks/' + assetName));
				isMissing = false;
			}
		}
		...
		if(isMissing) {
			...
			missingFileText.text = 'MISSING FILE: images/menus/storymenu/weeks/' + assetName + '.png';
		}
