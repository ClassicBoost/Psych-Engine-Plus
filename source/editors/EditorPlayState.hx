...
class EditorPlayState extends MusicBeatState
{
	...
	private var cpuControlledchart:Bool = false;
	...
	override function create()
	{
		...
		healthBar.createFilledBar(0xFFFF0000, 0xFF4CFF00);
		add(healthBar);
		...
		scoreTxt = new FlxText(0, healthBarBG.y + 36, FlxG.width, "Hits: 0 | Misses: 0 | Beat: 0 | Step: 0", 20);
		...
		beatTxt = new FlxText(10, healthBarBG.y + 66, FlxG.width, "Beat: 0", 20);
	...
	//	add(beatTxt);
		...
		stepTxt = new FlxText(10, healthBarBG.y + 96, FlxG.width, "Step: 0", 20);
	...
	//	add(stepTxt);
		...
		var tipText:FlxText = new FlxText(10, 0, 0, 'Press ESC to Go Back to Chart Editor', 16);
	...
	override function update(elapsed:Float) {
		...
		keyShit();
		scoreTxt.text = 'Hits: $songHits | Misses: $songMisses | Beat: $curBeat | Step: $curStep';
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			if(cpuControlledchart && (note.ignoreNote || note.hitCausesMiss)) return;
